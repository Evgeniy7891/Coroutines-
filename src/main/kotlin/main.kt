                ...
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            ...
                            println("Author - "+ Thread.currentThread().name + author)
                            val comments = getComments(client, post.authorId)
                                .map { comment ->
                                    async {
                                        CommentWithAuthor(comment, getAuthor(client, post.authorId))
                                        println("Comment Author - "+ Thread.currentThread().name + comment)
                                    }
                                }.awaitAll()
                        }
                    }.awaitAll()
                println("POSTS"+ Thread.currentThread().name  + posts)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
